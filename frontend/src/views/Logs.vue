  ...
  <div class="min-h-screen px-4 pb-8 pt-24">
    <div class="mx-auto max-w-7xl space-y-6">
      <Card>
        ...
        <div class="grid grid-cols-2 gap-6 md:grid-cols-4">
          <div class="text-center">
            ...
            <div class="mt-1 text-sm text-gray-600 dark:text-gray-400">总请求数</div>
          </div>
          <div class="text-center">
            <div class="text-3xl font-bold text-green-600">{{ stats?.success_count || 0 }}</div>
            <div class="mt-1 text-sm text-gray-600 dark:text-gray-400">成功次数</div>
          </div>
          <div class="text-center">
            <div class="text-3xl font-bold text-red-600">{{ stats?.failure_count || 0 }}</div>
            <div class="mt-1 text-sm text-gray-600 dark:text-gray-400">失败次数</div>
          </div>
          <div class="text-center">
            <div class="text-3xl font-bold text-primary">{{ stats?.success_rate || 0 }}%</div>
            <div class="mt-1 text-sm text-gray-600 dark:text-gray-400">成功率</div>
          </div>
        </div>
      </Card>
      ...
      <Card>
        <template #header>
          <div class="flex items-center space-x-3">
            <FunnelIcon class="h-6 w-6 text-primary" />
          ...
          <div class="min-w-[200px] flex-1">
            ...
            <div class="relative">
              <input
                ...
                type="date"
                :max="today"
                class="glass-input w-full cursor-pointer px-3 py-2 pr-10 dark:text-gray-100"
              />
              <CalendarIcon
          ...
          <table class="w-full">
            <thead>
              <tr class="border-b border-gray-200 dark:border-gray-700/50">
                <th
                  class="w-12 px-4 py-3 text-left text-sm font-semibold text-gray-700 dark:text-gray-300"
                ></th>
                <th
            ...
            <tbody>
              <template v-for="log in logs" :key="log.session_id">
                <tr
                  class="cursor-pointer border-b border-gray-100 transition-colors hover:bg-white/50 dark:border-gray-700/30 dark:hover:bg-white/5"
                ...
                <tr
                  v-if="expandedSessions.has(log.session_id)"
                  ...
                  <td colspan="5" class="px-4 py-4">
                    <div class="space-y-4">
                      <div>
                        ...
                        <div class="grid gap-2">
                          <div
                            ...
                            :key="idx"
                            class="flex items-center justify-between rounded-lg border border-gray-200 bg-white p-3 dark:border-slate-700 dark:bg-slate-800"
                          >
                            <div class="flex items-center space-x-3">
                              <span
                              ...
                              <span class="text-xs text-gray-500 dark:text-gray-400"
                                >HTTP {{ seat.status_code }}</span
                              >
                            </div>
                            <div class="flex items-center space-x-2">
                              <CheckCircleIcon v-if="seat.success" class="h-4 w-4 text-green-600" />
                              <XCircleIcon v-else class="h-4 w-4 text-red-600" />
                              <span
                                :class="seat.success ? 'text-green-600' : 'text-red-600'"
                                class="text-sm"
                              >
                                {{ seat.success ? '成功' : '失败' }}
                              </span>
                      ...
                      <div>
                        ...
                        <div
                          class="max-h-64 overflow-y-auto rounded-lg bg-gray-900/90 p-4 font-mono text-xs text-gray-300"
                        >
